...
<p align="center">
  ...
  &nbsp;·&nbsp;
  <a href="./README.pt-PT.md"><strong>Português (Portugal)</strong></a>
  <br /><br />
...
**Apenas browser** — chama `createAdaptiveCaret()` numa página com `document` (não suporta SSR sem guard).

> **Nota:** Este projecto foi maioritariamente *vibe coded* pelo **Designer** da equipa Square², não pelos programadores full stack. Serve como ponto de partida — adapta e valida no teu contexto antes de usar em produção.
...
Marca o texto que deve usar o modo caret com o selector por defeito (`[data-adaptive-caret-text], .text-hover`), por exemplo:

```html
<p class="text-hover">Hover para caret adaptativo</p>
```

Botões e âncoras (`button`, `a`, `[role="button"]` por defeito) activam o modo «interactivo» (círculo maior).
...
| `destroy()` | Remove o cursor, estilos de selecção desta instância, listeners e repõe o cursor nativo onde foi aplicado `cursor: none`. |

### `PACKAGE_BRAND`

Objecto constante com identidade do pacote: `displayName`, `packageName`, `websiteUrl`, `repositoryUrl`, `organization`.
...
| `interactiveSize` | `number \| string` | `42` | Diâmetro sobre alvos interactivos. |
...
| `cursorColor` | `string` | `#0f0f0f` | Cor do círculo por defeito / modo interactivo. |
| `caretColor` | `string` | `#2563eb` | Cor do caret. |
| `cursorShadow` | `string` | (sombra por defeito) | `box-shadow` do círculo. |
| `caretShadow` | `string` | (sombra por defeito) | `box-shadow` no modo caret. |
| `interactiveShadow` | `string` | (sombra por defeito) | `box-shadow` no modo interactivo. |
| `selectionBackground` | `string` | — | Cor de fundo de `::selection` (injeta CSS global). |
| `selectionColor` | `string` | — | Cor do texto em `::selection`. |
| `textSelector` | `string` | `[data-adaptive-caret-text], .text-hover` | Selector válido para `Element.closest()` — blocos de texto com caret. |
| `interactiveSelector` | `string` | `button, a, [role="button"]` | Selector para modo círculo maior (prioridade sobre texto). |
| `zIndex` | `number \| string` | `99999` | `z-index` do cursor. |

As constantes por defeito estão também em `DEFAULT_ADAPTIVE_CARET` (export nomeado).
...
Esconder o cursor nativo pode prejudicar utilizadores que dependem do ponteiro visível ou de tecnologias de apoio. Avalia `hideNativeCursor: false` durante testes ou oferece alternativa (por exemplo, respeitar `prefers-reduced-motion` numa versão futura).
